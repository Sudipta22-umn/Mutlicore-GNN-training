...
def buffer_config(buffer_size,num_clusters,total_vertices,feature_vect_size):
    ...
    ## need to perform feature vector partititon if is more than the minimum threshold

    if(num_feats_per_partition > num_features_per_cluster):
        print("need to reduce the cluster numbers")
    else:
        if (num_feats_per_partition > math.ceil((1/2)*num_features_per_cluster)):
            print("no  feature vector partitioning required")
            partition_num.append(1)
            buffer_cap.append(1*num_feats_per_partition)
        else:
            for i in range (2,17):
        ...
        #for gamma_point_1 in gamma_point_1_arr:
            for edge_percent in edge_percent_arr:
                for iter_iterval in iter_iterval_arr:
                    ...
                    vertices = pkl.load(open("OVCAR-8H_4_clusters_ORDERED_METIS.p", "rb"))
                    ...
                    dir_name="GCN_Aggregation_OVCAR-8H"+"_"+str(num_clusters)
