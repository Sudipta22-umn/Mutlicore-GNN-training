                ...
                #for gamma_point_1 in gamma_point_1_arr:
                    for edge_percent in edge_percent_arr:
                        for iter_iterval in iter_iterval_arr:
                            ...
                            dir_name="GraphSAGE_aggregation_"
                            ...
                            for iter in range(normal_iter_num):
                                ...
                                for i in range (len(on_chip_comm_cycle_count_list_f)):
                                    if (max(on_chip_comm_cycle_count_list_f[i],on_chip_comm_cycle_count_list_r[i])>max(mem_stall_per_machine_per_iter)*num_parts):
                                        on_chip_comm_cycle_count_list_total.append(max(on_chip_comm_cycle_count_list_f[i],on_chip_comm_cycle_count_list_r[i]))
                                    else:
                                        on_chip_comm_cycle_count_list_total.append(0)
                                ...
                                for k in range(num_mach):
                                    tmpk=dram_index[k]
                                    print(f"size of sub_graph {k} after BOTH OFF AND ON CHIP : {len(sub_graph[k])}")
                                    sim_log.write(f"size of sub_graph {k} after BOTH OFF AND ON CHIP : {len(sub_graph[k])}")
                                    sim_log.write('\n')
                                    off_chip_access=0
                                    off_chip_cycle_count=0
                                    while (len(sub_graph[k]) < buffer_cap): ## if the buffer is filled or the off-chip comm reaches the desired ratio)
                                        ...
                                        if (tmpk == dram_index[k]) or (len(sub_graph[k]) == buffer_cap):
                                            break
                                    dram_index[k] = tmpk
                                    print("num off-chip accesses sub_graph NORMAL mode SECOND ROUND", k,off_chip_access)
                                    sim_log.write(f'num off-chip accesses sub_graph NORMAL mode SECOND ROUND for machine  {k} is {off_chip_access}')
                                    sim_log.write('\n')
                                    print(f"size of of machine {k} after off chip SECOND ROUND : {len(sub_graph[k])}")
                                    sim_log.write(f"size of machine {k} after off chip SECOND ROUND: {len(sub_graph[k])}")
                                    sim_log.write('\n')
                                ...
                                for i in range(len(list_1)):
                                    ...
                                    for curr_node in sub_graph[list_1[i]]: 
                                        ...
                                        if (inter_cluster_gamma_flag[list_2[i]]==0):
                                            ...
                                            if (curr_node.inter_cluster_edge_cnt[0][list_2[i]] and  list_2[i] not in curr_node.cluster_sent_list):## add cehck for subgraph presence /sent list
                                              ...
                                              #print(f'!!!!INTER CLUSTER GAMMA FLAG ACTIVATED FOR MACHINE :{list_2[i]} with:{inter_cluster_gamma[list_2[i]]} at iter: {iter}')
                                              #sim_log.write(f'!!!!INTER CLUSTER GAMMA FLAG ACTIVATED FOR MACHINE :{list_2[i]} with: {inter_cluster_gamma[list_2[i]]} at iter:{iter}')
                                              sim_log.write('\n')
                                              if (curr_node.inter_cluster_edge_cnt[0][list_2[i]] > inter_cluster_gamma[list_2[i]] and  list_2[i] not in curr_node.cluster_sent_list):## add cehck for subgraph presence /sent list
                                                if (len(sub_graph[list_2[i]]) <buffer_cap):  ## if the buffer is filled or the off-chip comm reaches the desired ratio)
                                                    sub_graph[list_2[i]].append(curr_node)
                                    ...
                                    # on_chip_cycle_count=((sent_num*128)/307e9)/(.77e-9)
                                    # on_chip_comm_traffic_list_f.append(math.ceil(on_chip_cycle_count))
                                    # list_comm_per_iter_1.append(sent_num)    
                                ...
                                for i in range(len(list_2)):
                                  ...
                                  for curr_node in sub_graph[list_2[i]]: 
                                        ...
                                        if (inter_cluster_gamma_flag[list_1[i]]==0):
                                            ...
                                            #print(f'!!!!INTER CLUSTER GAMMA FLAG ACTIVATED FOR MACHINE :{list_1[i]} with:{inter_cluster_gamma[list_1[i]]} at iter: {iter}')
                                            #sim_log.write(f'!!!!INTER CLUSTER GAMMA FLAG ACTIVATED FOR MACHINE :{list_1[i]} with: {inter_cluster_gamma[list_1[i]]} at iter:{iter}')
                                            #sim_log.write('\n')
                                            if (curr_node.inter_cluster_edge_cnt[0][list_1[i]] > inter_cluster_gamma[list_1[i]] and  list_1[i] not in curr_node.cluster_sent_list):
                                                if (len(sub_graph[list_1[i]]) <buffer_cap):  ## if the buffer is filled or the off-chip comm reaches the desired ratio)
                                                    sub_graph[list_1[i]].append(curr_node)
                                                    ...
                                                    curr_node.cluster_sent_list.append(list_1[i])
                                  # on_chip_cycle_count=((sent_num*128)/307e9)/(.77e-9)              
                                  # on_chip_comm_traffic_list_r.append(math.ceil(on_chip_cycle_count))
                                  # if(i%4==0):
                                  #       on_chip_comm_cycle_count_list_r.append(max(on_chip_comm_traffic_list_r))
                                  #       on_chip_comm_traffic_list_r=[]
                                  # print(f"sent {list_2[i]}->{list_1[i]} {sent_num_2} nodes")
                                  # sim_log.write(f"sent {list_2[i]}->{list_1[i]} {sent_num} nodes")
                                  # sim_log.write('\n')
                                  # list_comm_per_iter_2.append(sent_num_2)   
                                # total_off_chip_comm_list.append(list_off_per_iter)    
                                # total_off_chip_cycle_list.append(list_off_cycle_per_iter)
                                # cycle_required_for_off_chip_comm_per_iter.append(max(list_off_cycle_per_iter)) ### jan 14
                                    ...
                                    ## if a subgraph lenght is less thna than the buffer size we need to fill up again through off-chip
                            ...
                            with open("./"+dir_name+"/"+sub_dir_name_1+"/"+sub_dir_name_2+"/"+folder_name+"/total_off_chip_comm_list_NORMAL.csv", "w", newline="") as f:
                                writer = csv.writer(f)
                                writer.writerows(data3)
                            ...
                            data_list_per_buffer_and_partitions.append(Total_cycles_requried_NORMAL_mem_stall)
