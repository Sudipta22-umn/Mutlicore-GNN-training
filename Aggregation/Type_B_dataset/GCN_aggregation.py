...
# -*- coding: utf-8 -*-
"""
Created on Wed Sep 21 16:12:41 2022
...
dir_name="dataset_vs_optimum_segments"

## call datasets in automated fashion
for dataset, hid_1, hid_2, total_vertices in dataset_info:
    dataset_optimum_segment_dict={}
    for num_clusters in num_cluster_list: ## for every machine config system
        ...
        top_data_list=[]
        ## for random mode
        random_top_data_list=[]
        ####
        config_cnt=0
        for buffer_cap in buffer_size_arr:
            normal_plus_random_cycles=0
            config_cnt +=1
                ...
                #for gamma_point_1 in gamma_point_1_arr:
                    for edge_percent in edge_percent_arr:
                        for iter_iterval in iter_iterval_arr:
                            ...
                            data_list_per_buffer_and_partitions=[]
                            random_data_list_per_buffer_and_partitions=[]
                            ...
                            dir_name="GCN_aggregation_"
                            sub_dir_name_1="num_clusters_"+str(num_clusters)
                            ...
                            for iter in range(normal_iter_num):
                                ...
                                cycle_required_for_PE_computation_per_iter.append(max(computation_cycle_list_per_machine_per_iter)) ## corrcted sept 21
                            ...
                            Total_mem_stall_cycles= math.ceil(sum(cycle_required_for_mem_stall_per_iter) + ((512*1024)/307e9/num_clusters)/(.77e-9))
                            ...
                            data_list_per_buffer_and_partitions.append(sum(cycle_required_for_PE_computation_per_iter))
                            data_list_per_buffer_and_partitions.append(Total_mem_stall_cycles)
                            data_list_per_buffer_and_partitions.append(sum(cycle_required_for_on_chip_comm_per_iter))
                            ...
                            normal_plus_random_cycles+=sum(cycle_required_for_PE_computation_per_iter)+Total_mem_stall_cycles+sum(cycle_required_for_on_chip_comm_per_iter)

                            ### start random mode here###
                            print("Starting RANDOM MODE")
                            sim_log.write("Starting RANDOM MODE")
                            sim_log.write('\n')
                            ### list of new verts
                            new_vert_2 = [[] for k in range(num_mach)]
                            new_vert_1 = [[] for k in range(num_mach)]
                            
                            for v in vertices:
                                v.inter_cluster_edge_cnt[1] = [0 for k in range(num_mach)]
                                v.total_edges = 0
                                if v.neighbor_list:
                                    new_vert_2[v.cluster_id].append(v)
                                    new_vert_1[v.cluster_id].append(v)
                             ## for ensuring buffer cap is always greater than new_vert_1 max length
                            new_vert_1_lenghts=[]
                            for item in new_vert_1:
                                new_vert_1_lenghts.append(len(item))
                                
                            if(max(new_vert_1_lenghts)>buffer_cap):
                                buffer_cap=max(new_vert_1_lenghts)
                            
                            ### check if random iteration is feasible or not
                            rand_feasible_flag=1
                            for k in range(num_mach):
                                if(len(new_vert_1[k])>buffer_cap):
                                    rand_feasible_flag=0
                                    print("random mode not FEASIBLE!!!!!")
                                    sim_log.write("random mode not FEASIBLE!!!!!")
                                    sim_log.write('\n')
                                  
                            if(rand_feasible_flag):
                                print("random mode feasible entering random mode SUCCESSFULLY!!!")
                                sim_log.write("random mode feasible entering random mode SUCCESSFULLY!!!")
                                sim_log.write('\n')
                                away_nodes = [set() for k in range(num_mach)]
                                for k in range(num_mach):
                                    for v in new_vert_2[k]:
                                        v.total_edges = len(v.neighbor_list)
                                        for nbr in v.neighbor_list:
                                            v.inter_cluster_edge_cnt[1][vertices[nbr].cluster_id] += 1
                                            if (vertices[nbr].cluster_id != k):
                                                vertices[nbr].inter_cluster_edge_cnt[1][k] += 1
                                            away_nodes[k].add(vertices[nbr])
                                                
                                for k in range(num_mach):
                                    for v in away_nodes[k]:
                                        new_vert_2[k].append(v)
                                    print('length of ', k, len(new_vert_2))
                                        
                                rand_iterations_per_machine=[0]*num_mach
                                # new_vert_2=[[] for _ in range(num_mach)]
                                for k in range(num_mach):
                                    # new_vert_2[k]=new_vert[k]+selected_vert_with_IC_edges_top[k]
                                    print(f'machine {k} has {len(new_vert_2[k])} vertices')
                                    print(f'machine {k} requires number of random iteration: {math.ceil(len(new_vert_2[k])/buffer_cap)}')
                                    sim_log.write(f'machine {k} has {len(new_vert_2[k])} vertices')
                                    sim_log.write(f'machine {k} requires number of random iteration: {math.ceil(len(new_vert_2[k])/buffer_cap)}')
                                    
                                    rand_iterations_per_machine[k]=math.ceil (len(new_vert_2[k])/buffer_cap)
                                    
                                rand_iteration_required=max(rand_iterations_per_machine)
                                one_shot_mode=[0]*num_mach
                                
                                sub_graph = []
                                for k in range(len(new_vert_2)):
                                    if (len(new_vert_2[k])<buffer_cap):
                                        sub_graph.append(new_vert_2[k])
                                       
                                    else:
                                        one_shot_mode[k]=1
                                        sub_graph.append(new_vert_2[k][0:buffer_cap])
                                        print(f'machine {k} requires more than one-shot mode')
                                        sim_log.write(f'machine {k} requires more than one-shot mode')
                                        sim_log.write('\n')
                                ## reinitilizaing variables
                                dram_index = [buffer_cap for _ in range(num_mach)]
                                all_nodes_done = True
                                iter = 0
                                #max_diff_list=[]
                                rem_list = [[] for _ in range(num_mach)]
                                ...
                                off_chip_comm_ratio=[1]*num_mach
                                ## list for cycle count
                                cycle_count_list_RAND=[0]*num_mach
                                import math
                                
                                
                                ## total node processed list of list
                                total_node_processed_per_iter_RAND=[]
                                ## total on chip communication list
                                total_on_chip_comm_list_1=[]
                                total_on_chip_comm_list_2=[]
                                total_on_chip_comm_list=[]
                                #####
                                total_off_chip_comm_list_RAND=[]
                                total_off_chip_cycle_list_RAND=[]
                                
                                ## sept 21 2022
                                top_computation_cycle_list_per_machine_RAND=[]
                                top_mem_stall_cycle_list_per_machine_RAND=[]
                                cycle_required_for_PE_computation_per_iter_RAND=[]
                                ## sept 21 2022
                                
                                ## added jan 15
                                cycle_required_for_on_chip_comm_per_iter_RAND=[]
                                cycle_required_for_off_chip_comm_per_iter_RAND=[]
                                cycle_required_for_mem_stall_per_iter_RAND=[]
                                ...
                                ##oct 22
                                no_change=False
                                # sim_log = open("simulation_log_amazon.txt", "a")
                                ## dec 4
                                
                                
                                ## fucntion definition of off-chip communication normal version
                                # def Off_chip_normal_version (ram_location, buffer_cap, vert_k, ):
                                
                                total_intra_cluster_edges_done_per_machine=[0]*num_mach
                                total_inter_cluster_edges_done_per_machine=[0]*num_mach
                                total_edges_done_per_machine=[0]*num_mach
                                removed_node_cnt=[0]*num_mach
                                list_removed_from_cluster=[[]for _ in range(num_mach)]
                                removed_node_list=[]
                                gamma=[0]*num_mach
                                ...
                                for item in new_vert_2[1]:
                                    check_list_2.add(item.node_identity)
                                    if (item.cluster_id==1 and item.neighbor_list):
                                        check_list.add(item.node_identity)
                                        for n in item.neighbor_list:
                                            check_list.add(n)
                                            
                                output_check=check_list_2-check_list
                                print(f'output_check:{output_check}')
                                    
                                removed_set=set()
                                left_set=set()
                                for iter in range(rand_iteration_required+1):
                                    print(f'current GAMMA:{gamma} at ITER:{iter}')
                                    sim_log.write(f'current GAMMA:{gamma} at ITER:{iter}')
                                    sim_log.write('\n')
                                    removed_node_cnt  = [0 for _ in range(num_mach)]
                                    ...
                                    computation_cycle_list_per_machine_per_iter_RAND=[]
                                    ...
                                    for k in range(num_mach):
                                        ...
                                        for curr_node in sub_graph_curr:
                                            if (curr_node.cluster_id == k):
                                                    # continue
                                                for nbr_iter in curr_node.neighbor_list[:]:
                                                    # if ((curr_node.cluster_id != k) and (vertices[nbr_iter].cluster_id != k)): ##  if ((curr_node.cluster_id != k) --> two way comm
                                                    #     continue
                                                    # if (curr_node.cluster_id != k):
                                                    #     continue
                                                    nbr = vertices[nbr_iter]
                                            ...
                                            if curr_node.cluster_id != k and curr_node.inter_cluster_edge_cnt[1][k] == 0 :
                                                ...
                                                #sub_graph_curr.remove(curr_node) ## COMMENTED OCT 27
                                                removed_nodes.append(curr_node)## ADDED OCT 27
                                                removed_node_list.append(curr_node.node_identity)
                                                if(k in curr_node.cluster_sent_list):
                                                    #curr_node.cluster_sent_list.remove(k) 
                                                    list_removed_from_cluster[k].append(curr_node.node_identity)
                                        ...
                                        # for curr_node in removed_nodes:
                                        #     if k == 1:
                                        #         removed_set.add(curr_node.node_identity)
                                        #     sub_graph[k].remove(curr_node)
                                        #     if (k not in curr_node.sub_graph_presence):
                                        #           print(k, curr_node.node_identity, curr_node.sub_graph_presence, curr_node.neighbor_list)
                                        #     curr_node.sub_graph_presence.remove(k) ## Addec DEC 8
                                        ...
                                        cycle_count_list_RAND[k]+=math.ceil(max(num_egdes_processed_list)/256) ## for cycle counting
                                    ...
                                    #########
                                            ...
                                            ## Degree information collection after each iteration
                                            
                                    
                                    
                                    ## Added sept 21
                                    list_refetch_off_chip_RAND=[]
                                    
                                    list_num_refetces_per_iter_RAND=[]
                                    for k in range(num_mach):
                                          list_num_refetces_per_iter_RAND.append(len(sub_graph[k]))
                                          list_refetch_off_chip_RAND.append(math.ceil(((len(sub_graph[k])*feature_vector_partition_len)/(307e9/num_clusters))/(.77e-9)))
                                          
                                  
                                    #cycle_required_for_off_chip_comm_per_iter_refetch.append(max(list_refetch_off_chip_RAND))
                                    ## compute the blank space
                                    for k in range(num_mach):
                                        avaiable_buffer_space_per_iter.append(buffer_cap-len(sub_graph[k]))
            
                                    for k in range(num_mach):
                                        computation_cycle_list_per_machine_per_iter_RAND.append(math.ceil(num_egdes_processed_list[k]/num_edge_processed_per_memory_access)*16) ## 64 items in total and 4 macc units per CPE
                                    top_computation_cycle_list_per_machine_RAND.append(computation_cycle_list_per_machine_per_iter)
                                    
                                    cycle_required_for_PE_computation_per_iter_RAND.append(max(computation_cycle_list_per_machine_per_iter_RAND))
                                    
                                 
                                   
                                    ########################################### OFF-CHIP #############
                                    list_off_per_iter=[]
                                    list_off_cycle_per_iter_RAND=[]
                                    
                                    for k in range(num_mach):
                                        if(rand_iterations_per_machine[k]>1):
                                        #print("NORMAL HERE")
                                            print(f"size of sub_graph {k} before off chip : {len(sub_graph[k])}")
                                            sim_log.write(f"size of sub_graph {k} before off chip : {len(sub_graph[k])}")
                                            sim_log.write('\n')
                                            tmpk = dram_index[k]
                                            off_chip_access=0
                                            while (len(sub_graph[k]) < buffer_cap): ## if the buffer is filled or the off-chip comm reaches the desired ratio)
                                                if ( (new_vert_2[k][tmpk].inter_cluster_edge_cnt[1][k]>0 or(new_vert_2[k][tmpk].cluster_id==k and new_vert_2[k][tmpk].total_edges>0)) and k not in new_vert_2[k][tmpk].sub_graph_presence):
                                                    sub_graph[k].append(new_vert_2[k][tmpk]) 
                                                    new_vert_2[k][tmpk].sub_graph_presence.add(k) ### ADDED oct 26
                                                    dram_access_list[k]+=1
                                                    off_chip_access+=1
                                                elif (k not in new_vert_2[k][tmpk].sub_graph_presence and k==1):
                                                    left_set.add(new_vert_2[k][tmpk].node_identity)
                                                    
                                                tmpk = (tmpk + 1) % len(new_vert_2[k])
                                                if (tmpk == dram_index[k]) or (len(sub_graph[k]) == buffer_cap):
                                                    break
                                            dram_index[k] = tmpk
                                            print("num off-chip accesses sub_graph RANDOM mode", k,off_chip_access)
                                            print(f"size of sub_graph {k} after off chip in RANDOM mode: {len(sub_graph[k])}")
                                            # print("num off-chip accesses sub_graph RANDOM mode", k,off_chip_access)
                                            # print(f"size of sub_graph {k} after off chip in RANDOM mode: {len(sub_graph[k])}")
                                            #list_off_per_iter.append(off_chip_access)
                                        else:
                                        ...
                                        off_chip_cycle_count=((off_chip_access*feature_vector_partition_len)/(307e9*.01/num_clusters))/(.77e-9)
                                        list_off_per_iter.append(off_chip_access)
                                        list_off_cycle_per_iter_RAND.append(off_chip_cycle_count)
                                        
                                    total_off_chip_comm_list_RAND.append(list_off_per_iter)     
                                    total_off_chip_cycle_list_RAND.append(list_off_cycle_per_iter)
                                    cycle_required_for_off_chip_comm_per_iter_RAND.append(max(list_off_cycle_per_iter_RAND)) ### jan 14
                                    
                            
                                    mem_stall_per_machine_per_iter_RAND=[]
                                    for k in range(num_mach):
                                        if(list_off_cycle_per_iter_RAND[k]+list_refetch_off_chip_RAND[k]-computation_cycle_list_per_machine_per_iter_RAND[k] >0):
                                            mem_stall_per_machine_per_iter_RAND.append(list_off_cycle_per_iter_RAND[k]+list_refetch_off_chip_RAND[k]-computation_cycle_list_per_machine_per_iter_RAND[k])
                                        else:
                                            mem_stall_per_machine_per_iter_RAND.append(0)
                                    top_mem_stall_cycle_list_per_machine_RAND.append(mem_stall_per_machine_per_iter)
                                    cycle_required_for_mem_stall_per_iter_RAND.append(max(mem_stall_per_machine_per_iter)*num_parts)
                                    ...
                                    for k in range(num_mach):
                                      ...
                                      #print(f'iter : {iter} k : {k} done : {count_done}')
                                      total_processed_node_list.append(count_done)
                                    total_node_processed_per_iter_RAND.append(total_processed_node_list)
                                    
                                diff_set = left_set - removed_set
                                print(f'diff_set: {diff_set}')  
                                ...
                                for k in range (num_mach):
                                    for item in vert[k]:
                                        if (item.total_edges==0):
                                            gt_5+=1         
                                print("gt_5:",gt_5)
                                if(len(vertices)==gt_5):
                                    print("ALL DONE!! THANK YOU!! HAVE A NICE DAY!!")
                                    sim_log.write("ALL DONE!! THANK YOU!! HAVE A NICE DAY!!")
                                    sim_log.write('\n')
                                
                                ## FOR GETTING THE DATA OUT
                                data3 = total_off_chip_comm_list_RAND
                                pkl.dump( data3, open( "./"+dir_name+"/"+sub_dir_name_1+"/"+sub_dir_name_2+"/"+folder_name+"total_off_chip_comm_list_RAND.p", "wb" ) )
                                with open ("./"+dir_name+"/"+sub_dir_name_1+"/"+sub_dir_name_2+"/"+folder_name+"total_off_chip_comm_list_RAND.csv", "w", newline="") as f:
                                    writer = csv.writer(f)
                                    writer.writerows(data3)
                                
                                rand_iter_req_top=[]
                                rand_iter_req_top.append(rand_iterations_per_machine)
                                data3 = rand_iter_req_top
                                with open("./"+dir_name+"/"+sub_dir_name_1+"/"+sub_dir_name_2+"/"+folder_name+"RAND_iter_per_mach.csv", "w", newline="") as f:
                                    writer = csv.writer(f)
                                    writer.writerows(data3)
                                    
                                data3 = total_node_processed_per_iter_RAND
                                pkl.dump( data3, open( "./"+dir_name+"/"+sub_dir_name_1+"/"+sub_dir_name_2+"/"+folder_name+"total_node_processed_per_iter_RAND.p", "wb" ) )
                                with open ("./"+dir_name+"/"+sub_dir_name_1+"/"+sub_dir_name_2+"/"+folder_name+"total_node_processed_per_iter_RAND.csv", "w", newline="") as f:
                                    writer = csv.writer(f)
                                    writer.writerows(data3)
                                
                                data = cycle_required_for_PE_computation_per_iter_RAND
                                pkl.dump( data, open( "./"+dir_name+"/"+sub_dir_name_1+"/"+sub_dir_name_2+"/"+folder_name+"cycle_required_for_PE_computation_per_iter_RAND.p", "wb" ) )
                                with open ("./"+dir_name+"/"+sub_dir_name_1+"/"+sub_dir_name_2+"/"+folder_name+"/inter_edge_done_top_list_NORMAL.csv", "w", newline="") as f:
                                    writer = csv.writer(f)
                                    writer.writerows(map(lambda x: [x], data))
                                
                                data = cycle_required_for_off_chip_comm_per_iter_RAND
                                pkl.dump( data, open( "./"+dir_name+"/"+sub_dir_name_1+"/"+sub_dir_name_2+"/"+folder_name+"cycle_required_for_off_chip_comm_per_iter_RAND.p", "wb" ) )
                                with open ("./"+dir_name+"/"+sub_dir_name_1+"/"+sub_dir_name_2+"/"+folder_name+"/inter_edge_done_top_list_NORMAL.csv", "w", newline="") as f:
                                    writer = csv.writer(f)
                                    writer.writerows(map(lambda x: [x], data))
                                    
                                print("Total cycle required for PE computation RAND:", sum(cycle_required_for_PE_computation_per_iter_RAND))
                                print("Total cycle required for off-chip communication RAND:", sum(cycle_required_for_mem_stall_per_iter_RAND))
                                print("Total cycle required for on-chip communication RAND:", sum(cycle_required_for_on_chip_comm_per_iter_RAND))
                                Total_cycles_requried_RAND=sum(cycle_required_for_PE_computation_per_iter_RAND)+sum(cycle_required_for_mem_stall_per_iter_RAND)+((512*1024)/(307e9*.01)/num_clusters)/(.77e-9)
                                #print("Total overall cycles required in NORMAL:",Total_cycles_requried_NORMAL)
                            
                                sim_log.write(f"Total cycle required for PE computation RAND:{sum(cycle_required_for_PE_computation_per_iter_RAND)}")
                                sim_log.write('\n')
                                sim_log.write(f"Total cycle required for off-chip communication RAND:{sum(cycle_required_for_off_chip_comm_per_iter_RAND)}")
                                sim_log.write('\n')
                                sim_log.write(f"Total cycle required for on-chip communication RAND:{sum(cycle_required_for_on_chip_comm_per_iter_RAND)}")
                                sim_log.write('\n')
                          
                                sim_log.write(f"Total overall cycles required in RAND:{Total_cycles_requried_RAND}")
                                sim_log.write('\n')
                                
                                Random_PE_cycle_list.append(sum(cycle_required_for_PE_computation_per_iter_RAND))
                                Random_off_chip_cycle_list.append(sum(cycle_required_for_mem_stall_per_iter_RAND))
                                Random_total_cycle_list.append(Total_cycles_requried_RAND)
                                
                                
                                random_data_list_per_buffer_and_partitions.append(buffer_cap)
                                random_data_list_per_buffer_and_partitions.append(num_parts)
                                random_data_list_per_buffer_and_partitions.append(sum(cycle_required_for_PE_computation_per_iter_RAND))
                                random_data_list_per_buffer_and_partitions.append(sum(cycle_required_for_mem_stall_per_iter_RAND)+((512*1024)/(307e9*.01)/num_clusters)/(.77e-9))
                                random_data_list_per_buffer_and_partitions.append(Total_cycles_requried_RAND)
                                #random_print(total_edge_done_top_list[-1])
                                #random_data_list_per_buffer_and_partitions.append(sum(total_edge_done_top_list[-1])/len(total_edge_done_top_list[-1]))
                                random_top_data_list.append(random_data_list_per_buffer_and_partitions)
                                normal_plus_random_cycles+=Total_cycles_requried_RAND
             
                  
        
                            fetaure_segments_config_dict[num_parts]=normal_plus_random_cycles
        ...
        data_2=random_top_data_list
        print(data_2)
        with open("./"+dir_name+"/"+sub_dir_name_1+"/"+sub_dir_name_2+"/random_top_data.csv", "w", newline="") as f:
             writer = csv.writer(f)
             writer.writerows(data_2)
    
    dataset_optimum_segment_dict[dataset]=min(fetaure_segments_config_dict,key=fetaure_segments_config_dict.get)

## wirte out the contents of the dictionry into
optimum_feature_segment_list=list(dataset_optimum_segment_dict.items())
data = optimum_feature_segment_list
#pkl.dump( data, open( "./"+dir_name+"/"+folder_name+"cycle_required_for_PE_computation_per_iter.p", "wb" ) )
with open("./"+dir_name+"/"+"optimum_feature_segment.csv", "w", newline="") as f:
    writer = csv.writer(f)
    writer.writerows(data)
